...
const ScoreWindow = () => {
    ...
    useEffect(() => {
        ...
        const appendImageData = () => {
            if (images.length > 0) {
                const message = images.map(data => `ID: ${data.fileName}, Width: ${data.width}, Height: ${data.height}, 
                Position: ${data.x.toFixed(0)} ${data.y.toFixed(0)}`).join('<br>');
